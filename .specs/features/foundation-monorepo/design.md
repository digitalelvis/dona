...
ESLint catches *imports*. We additionally guard `package.json` `dependencies`/`devDependencies` so a package can't even declare a forbidden SDK. This catches drift such as someone adding `@aws-sdk/client-dynamodb` to `packages/ports/package.json` without yet importing it. Pseudocode:
